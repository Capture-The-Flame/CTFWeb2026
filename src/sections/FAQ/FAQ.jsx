...
const FAQ_CONTENT = {
  ...
  4: { 
    ...
    laptop: { 
      ...
      textStyles: { top: "10%", left: "12%", width: "75%", fontSize: "1.3vw", bodySize: "1.1vw" },
      button: {bottom: "20%", right: "20%"} 
    },
    mobile: { 
...
export default function FAQ({ onOpen, onClose }) {
  ...
  {/* lock scrolling for board popup - iOS friendly */}
  useEffect(() => {
    if (openBoard || openPage) {
        ...
        <div className="board-small-frame" onClick={() => {setOpenBoard(true); onOpen?.();} }>
          ...
          <div className="board-small-overlay">
            {SMALL_BOARD_PAGES.map((page, index) => (
              <img key={index} src={page.img} className="mini-page" 
                style={{ top: page.top, left: page.left, width: page.width, transform: `rotate(${page.rot})` }} alt="" />
            ))}
          </div>
        </div>
      </div>

      {/* portal for board popup - iOS friendly */}
      {createPortal(
        <>
          {openBoard && (
                ...
                <div className="board-image-wrapper">
                  ...
                  <div className="board-overlay">
                    {(isMobile ? SECTIONS[currSection] : Object.keys(FAQ_CONTENT)).map((key) => {
                      ...
                      return (
                        <div key={key} className="page" onClick={() => setOpenPage(key)}
                          style={{ transform: `rotate(${config.styles.rot || config.styles.pageRot})`, top: config.styles.top, left: config.styles.left, width: config.styles.width }}>
                          <img src={item.img} className="page-image" />
                          <div className="page-text-overlay" style={{ transform: `rotate(${config.styles.textRot || '0deg'})`, top: config.textStyles.top, left: config.textStyles.left, width: config.textStyles.width }}>
                            <h3 className="page-title-mini" style={{ fontSize: isMobile ? `clamp(12px, ${config.textStyles.fontSize}, 24px)` : `clamp(10px, ${config.textStyles.fontSize}, 14px)` }}>{item.text.title}</h3>
                            <p className="page-body-mini" style={{ fontSize: isMobile ? `clamp(10px, ${config.textStyles.bodySize}, 20px)` : `clamp(10px, ${config.textStyles.bodySize}, 15px)` }}>{item.text.body}</p>
                          </div>
                        </div>
                      );
                    })}
            ...
            <div className="page-popup" onClick={() => setOpenPage(null)}>
              <div className="page-popup-page" onClick={(e) => e.stopPropagation()}>
                ...
                <button className="page-back-btn" onClick={() => setOpenPage(null)}
                  style={{ bottom: isMobile ? FAQ_CONTENT[openPage].mobile.button?.bottom : FAQ_CONTENT[openPage].laptop.button?.bottom, right: isMobile ? FAQ_CONTENT[openPage].mobile.button?.right : FAQ_CONTENT[openPage].laptop.button?.right }}>
                  Back
                </button>
              </div>
            </div>
